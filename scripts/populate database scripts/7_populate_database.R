...
source_files <- data.frame(
  path      = character(),
  source_db = character(),   # parent folder name — used for display only
...
# Force-close any open RSQLite connections in this R session before touching the file
local({
  open_cons <- tryCatch(DBI::dbListConnections(RSQLite::SQLite()), error = function(e) list())
  if (length(open_cons) > 0) {
    lapply(open_cons, function(con) try(DBI::dbDisconnect(con), silent = TRUE))
    cat(sprintf("Closed %d open RSQLite connection(s).\n", length(open_cons)))
  }
})
...
if (file.exists(OUTPUT_PATH)) {
  ...
  removed <- file.remove(OUTPUT_PATH)
  stop_if(!removed, paste(
    "Cannot delete existing master DB — file is locked (open in another R session or Shiny app?).",
    "\n  Close all connections to:", OUTPUT_PATH,
...
for (i in seq_len(nrow(source_files))) {
  tryCatch({
    ...
    dbDisconnect(con)
    if (nrow(rows) == 0) next
    rows$file_path <- source_files$path[i]
...
assessor_id_map <- all_assessors_df %>%
  ...
  select(file_path, old_idAssessor = idAssessor, new_idAssessor)
...
pest_id_map <- all_pests_df %>%
  ...
  mutate(new_idPest = coalesce(new_idPest,
    pests_to_insert$new_idPest[match(paste(file_path, idPest),
                                     paste(pests_to_insert$file_path, pests_to_insert$idPest))])) %>%
  select(file_path, old_idPest = idPest, new_idPest)
    ...
    tryCatch({
      ...
      for (j in seq_len(nrow(rows))) {
        ...
        new_ass <- assessment_id_map %>%
          filter(file_path == fp, old_idAssessment == row$idAssessment) %>%
  ...
  tryCatch({
    ...
    for (j in seq_len(nrow(rows))) {
      ...
      entrypath_id_map <- rbind(entrypath_id_map, data.frame(
        file_path = fp, old_idEntryPathway = row$idEntryPathway,
...
for (i in seq_len(nrow(source_files))) {
  ...
  rows <- tryCatch({
    con <- dbConnect(SQLite(), fp)
    r <- dbReadTable(con, "pathwayAnswers")
    dbDisconnect(con)
    r
  }, error = function(e) {
    try(dbDisconnect(con), silent = TRUE)
  ...
  for (j in seq_len(nrow(rows))) {
    ...
    new_ep <- entrypath_id_map %>%
      filter(file_path == fp, old_idEntryPathway == row$idEntryPathway) %>%
    ...
    result <- tryCatch({
      ...
      warn_skip(paste("pathwayAnswers row", j, "from", src, ":", e$message))
      0L
    })
    if (result == 1L) n_pa <- n_pa + 1L else n_pa_dup <- n_pa_dup + 1L
  }
  if (n_pa_dup > 0)
    cat(sprintf("  [WARN] %d duplicate pathway answer(s) skipped from %s\n",
                n_pa_dup, basename(fp)))
}
...
n_sim_read <- setNames(integer(nrow(source_files)), source_files$path)
...
for (i in seq_len(nrow(source_files))) {
  ...
  tryCatch({
    ...
    dbDisconnect(con)
    if (nrow(rows) == 0) next
    n_sim_read[fp] <- n_sim_read[fp] + nrow(rows)
    for (j in seq_len(nrow(rows))) {
      row <- rows[j, ]
      new_ass <- assessment_id_map %>%
        filter(file_path == fp, old_idAssessment == row$idAssessment) %>%
      ...
      simulation_id_map <- rbind(simulation_id_map, data.frame(
        file_path = fp, old_idSimulation = row$idSimulation,
  ...
  tryCatch({
    ...
    for (j in seq_len(nrow(rows))) {
      ...
      new_sim <- simulation_id_map %>%
        filter(file_path == fp, old_idSimulation == row$idSimulation) %>%
...
per_source <- source_files %>%
  mutate(
    ass_read   = n_ass_read[path],
    ass_merged = sapply(path, function(fp) sum(assessment_id_map$file_path == fp)),
    ass_dropped= ass_read - ass_merged,
    sim_read   = n_sim_read[path],
    sim_merged = sapply(path, function(fp) sum(simulation_id_map$file_path == fp)),
    sim_dropped= sim_read - sim_merged,
    status     = case_when(
      ass_read == 0                          ~ "NO ASSESSMENTS READ",
      ass_dropped > 0 | sim_dropped > 0      ~ "PARTIAL (rows dropped)",
      TRUE                                   ~ "ok"
    )
  )

fmt <- "  %-40s  ass %d/%d  sim %d/%d  [%s]\n"
for (i in seq_len(nrow(per_source))) {
  r <- per_source[i, ]
  cat(sprintf(fmt,
    paste0(r$source_db, " / ", basename(r$path)),
    r$ass_merged, r$ass_read,
    r$sim_merged, r$sim_read,
    r$status))
}

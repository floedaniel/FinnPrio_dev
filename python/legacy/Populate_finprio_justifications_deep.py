...
DEFAULT_DB_PATH = r"C:\Users\dafl\OneDrive - Folkehelseinstituttet\FinnPrio\FinnPRIO_development\databases\daniel_database_2026\daniel_v001_2026-04-14T13-09-15.db"
...
os.environ.update({
    ...
    # Depth     : recursion levels — each branch dives deeper, following leads.
    # Concurrency: async paths running simultaneously (bounded by API rate limits).
    # Trade-off : cost/time grow roughly with breadth * depth. Defaults: 4/2/4.
    # Our setting: depth=3 for richer chains of evidence; breadth=4 covers aspects;
    # concurrency=4 stays within typical OpenAI/Tavily rate limits.
    "DEEP_RESEARCH_BREADTH": "4",
    "DEEP_RESEARCH_DEPTH": "2", # ← reduced from 3; saves 75% cost/time
...
async def research_justification(pest_name: str, question_code: str, question_text: str,
    ...
    def on_progress(progress):
        # Called synchronously by gpt_researcher — must not be async.
        query = (progress.current_query or "")[:80]
        print(
